...
describe('data_generator fetcher', () => {
    ...
    describe('when using data type fields config', () => {
        it('should throw if invalid field config', async () => {
            expect.hasAssertions();
            try {
                await makeFetcherTest({
                    data_type_fields: { foo: 'foo' }
                });
            } catch (error: any) {
                expect(error?.message).toInclude('Field \\"foo\\" must have a valid field type');
            }
        });

        it('should be work when passed a data type config', async () => {
            const test = await makeFetcherTest({
                mode: 'data_type',
                data_type_config: {
                    fields: {
        ...
        it('should be work when using the default data type config', async () => {
            const test = await makeFetcherTest({ mode: 'data_type' });
